...
<template>
      ...
      <table class="tw-border-collapse tw-border-hidden">
        <tr v-for="(row, rowIndex) in rows" :key="rowIndex">
          <td v-for="(item, colIndex) in row" :key="colIndex"
            ...
            <div>
              <h3 class="tw-text-center tw-border-b tw-border-black "> {{ item.start_date.toLocaleDateString() }} - das {{ item.start_time }} ás {{ item.end_time }} </h3>
              <div v-for="discount in item.discounts" class="tw-whitespace-nowrap">Pizza tamanho {{ discount.sizes }} = {{ discount.percentage }}% de desconto</div>
            </div>
          </td>
      </tr>
    </table>
  ...
  <!-- <div class="tw-flex tw-place-items-center tw-flex-col">
    <div class="tw-flex tw-place-items-center tw-flex-col">
      <h1 class="tw-underline">Promoções</h1>
      <table class="tw-flex tw-flex-wrap tw-w-10/12 tw-ml-28 [&>td:last-child]:tw-w-lst">
        <td v-for="event in sales" class="tw-w-80 tw-min-w-80 tw-max-w-80 tw-bg-gray-100 tw-ml-2/2 tw-mt-2/2 tw-mb-hpx tw-border-2 tw-px-2 tw-pb-3 tw-border-black">
          <h3>
            {{ event.start_date.toLocaleDateString() }} - das {{ event.start_time }} ás {{ event.end_time }}
          </h3>
              <div>
                <div v-for="discount in event.discounts" class="tw-whitespace-nowrap">Pizza tamanho {{ discount.sizes }} = {{ discount.percentage }}% de desconto</div>
              </div>
            </td>
          </table>
        </div> -->
